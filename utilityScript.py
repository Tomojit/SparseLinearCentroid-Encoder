...
def getApplicationData(dataSetName,partition):
	...
	elif dataSetName.upper()=='GLIOMA':
		
		if not(os.path.isdir('GLIOMA')):  # files were not unzipped
			files = ['./Data/GLIOMA.zip']
			with ZipFile('./Data/GLIOMA.zip', 'r') as zip:
				zip.extractall()
		trnSet = pickle.load(open('./GLIOMA/Partition'+str(partition)+'/trnData.p','rb'))
		tstSet = pickle.load(open('./GLIOMA/Partition'+str(partition)+'/tstData.p','rb'))
